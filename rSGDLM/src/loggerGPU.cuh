...
inline bool cublasAssert(cublasStatus_t code, const char *file, int line) {
  if (code != CUBLAS_STATUS_SUCCESS) {
    ...
    WARNING_LOGGER << "cublasAssert (GPU " << no_gpu
                   << "): " << cublasGetErrorString(code) << " in "
...
inline bool curandAssert(curandStatus_t code, const char *file, int line) {
  if (code != CURAND_STATUS_SUCCESS) {
    int no_gpu = -1;
    cudaGetDevice(&no_gpu);
    WARNING_LOGGER << "curandAssert (GPU " << no_gpu
                   << "): " << curandGetErrorString(code) << " in "
                   << std::string(file) << ", line " << line << "." << ENDL;
  }
...
inline bool cudaAssert(cudaError_t code, const char *file, int line) {
  if (code != cudaSuccess) {
    int no_gpu = -1;
    cudaGetDevice(&no_gpu);
    WARNING_LOGGER << "cudaAssert (GPU " << no_gpu
                   << "): " << cudaGetErrorString(code) << " in "
                   << std::string(file) << ", line " << line << "." << ENDL;
  }
...
inline bool mexGPUAssert(int code, const char *file, int line) {
  if (code != MX_GPU_SUCCESS) {
    WARNING_LOGGER
        << "mexGPUAssert: could not initialize the Mathworks GPU API in "
        << std::string(file) << ", line " << line << "." << ENDL;
  }

  return code == MX_GPU_SUCCESS;
}
#define mxGPUErrchk(ans)                                                       \
  {                                                                            \
    mexGPUAssert((ans), __FILE__, __LINE__);                                   \
  }
#endif
#endif
