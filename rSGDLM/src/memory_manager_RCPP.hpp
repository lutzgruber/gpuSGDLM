...
public:
  memory_manager_RCPP() : memory_manager() {
  ...
  T *getData(const vector_type &vec) {
    ...
    for (typename vector_type::const_iterator it = vec.begin(); it != vec.end();
         it++, running_pointer++) {
  ...
  template <typename T, typename vector_type>
  static vector_type getRcppVector(const T *host_data_ptr,
                                   std::size_t no_elements) {
    return getRcppVector<T, vector_type>(host_data_ptr, 1, &no_elements);
  }

  template <typename T, typename vector_type>
  static vector_type getRcppVector(const T *host_data_ptr, std::size_t ndims,
                                   const std::size_t *dims) {
    std::vector<std::size_t> dim(ndims);

    for (std::vector<std::size_t>::iterator it = dim.begin(); it != dim.end();
         it++) {
      *it = *dims;
      dims++;
    }
  ...
  static vector_type getRcppVector(const T *host_data_ptr,
    ...
    for (typename vector_type::iterator it = vec.begin(); it != vec.end();
         it++) {
      *it = (T)*host_data_ptr;
      host_data_ptr++;
    }
  ...
  template <typename T>
  static std::vector<T> getSTDVector(const T *host_data_ptr,
                                     std::size_t no_elements) {
  ...
  template <typename T>
  static std::vector<T> getSTDVector(const T *host_data_ptr, std::size_t ndims,
                                     const std::size_t *dims) {
