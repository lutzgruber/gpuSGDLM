...
public:
  ...
  const NULLstream &operator<<(const endl &value) const { return *this; }
};
...
/*inline std::string log_dims(std::string name, size_t ndims, const size_t*
 dims) { std::stringstream ss;
