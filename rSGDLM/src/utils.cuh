 ...
 */
template <typename T>
inline void makePtrArray(T *in, size_t no_matrices, size_t matrix_size,
                         T **out) {
 ...
 */
template <typename T>
inline void cpyPtrArrayToDevice(T **host_ptr, T **dev_ptr, size_t no_matrices) {
  cudaErrchk(cudaMalloc((void **)&dev_ptr, no_matrices * sizeof(T *)));
  cudaErrchk(cudaMemcpy(dev_ptr, host_ptr, no_matrices * sizeof(T *),
