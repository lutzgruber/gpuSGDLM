...
template <typename DOUBLE> class HostWrapper {
public:
  virtual ~HostWrapper() {}
  ...
  virtual void initForecastPriors() = 0;

  virtual bool isPrior() const = 0;

  virtual void isPrior(bool is_prior) = 0;

  virtual void getParameters(DOUBLE *host_data_m_t, DOUBLE *host_data_C_t,
                             DOUBLE *host_data_n_t,
                             DOUBLE *host_data_s_t) const = 0;

  virtual void setParameters(const DOUBLE *host_data_m_t,
                             const DOUBLE *host_data_C_t,
                             const DOUBLE *host_data_n_t,
                             const DOUBLE *host_data_s_t) = 0;

  virtual void getDiscountFactors(DOUBLE *host_data_beta,
                                  DOUBLE *host_data_delta) const = 0;

  virtual void setDiscountFactors(const DOUBLE *host_data_beta,
                                  const DOUBLE *host_data_delta) = 0;

  virtual void getEvolutionMatrix(DOUBLE *host_data_G_t) const = 0;

  virtual void setEvolutionMatrix(const DOUBLE *host_data_G_t) = 0;

  virtual void getParentalSets(unsigned int *host_data_p,
                               unsigned int *host_data_sp_indices) const = 0;

  virtual void setParentalSets(const unsigned int *host_data_p,
                               const unsigned int *host_data_sp_indices) = 0;

  virtual void computePrior(bool evolve_forecast_variables) = 0;

  virtual void computePrior(const DOUBLE *host_data_G_tp1,
                            bool evolve_forecast_variables) = 0;
  ...
  virtual void computePosterior(const DOUBLE *host_data_y_t,
                                const DOUBLE *host_data_F_t) = 0;

  virtual void computeVBPosterior(DOUBLE *host_data_mean_m_t,
                                  DOUBLE *host_data_mean_C_t,
                                  DOUBLE *host_data_mean_n_t,
                                  DOUBLE *host_data_mean_s_t,
                                  DOUBLE *host_data_IS_weights,
                                  DOUBLE *host_sum_det_weights) = 0;
};
