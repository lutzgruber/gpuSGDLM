...
inline void curandGenerateUniformX(curandGenerator_t generator,
...
inline void curandGenerateNormalX(curandGenerator_t generator,
...
inline void curandGenerateNormalX(curandGenerator_t generator, float *outputPtr,
                                  size_t n, float mean, float stddev) {
  curandErrchk(curandGenerateNormal(generator, outputPtr, n, mean, stddev));
}

template <typename DOUBLE>
inline void sampleGamma(curandGenerator_t gen, memory_manager_GPU &MEM,
                        size_t m, size_t n, const DOUBLE *alphas,
  ...
  // #ifdef MATLAB_GPU
  //	mxGPUArray* mx_uniforms = MEM._mxGPUCreateGPUArray(1,
  //&n_input_random_numbers, mx_class_id<DOUBLE>::id, mxREAL,
  // MX_GPU_DO_NOT_INITIALIZE); 	DOUBLE* uniforms = (DOUBLE*)
  // mxGPUGetData(mx_uniforms); #else
  ...
  // #ifdef MATLAB_GPU
  //	mxGPUArray* mx_normals = MEM._mxGPUCreateGPUArray(1,
  //&n_input_random_numbers, mx_class_id<DOUBLE>::id, mxREAL,
  // MX_GPU_DO_NOT_INITIALIZE); 	DOUBLE* normals = (DOUBLE*)
  // mxGPUGetData(mx_normals); #else
  DOUBLE *normals = NULL;
  ...
  //	mxGPUArray* mx_fill_indices = MEM._mxGPUCreateGPUArray(2, dim_fill,
  // mxUINT32_CLASS, mxREAL, MX_GPU_INITIALIZE_VALUES); 	unsigned int*
  // data_fill_indices = (unsigned int*) mxGPUGetData(mx_fill_indices); #else
  ...
  make_gamma<<<numBlocks, threadsPerBlock,
               m * numBlocks.x * sizeof(unsigned int)>>>(
      m, n, n_loops, fill_indices, uniforms, normals, alphas, betas, gammas);
 ...
 */
template <typename DOUBLE>
inline void sampleGamma2(curandGenerator_t gen, size_t m, size_t n,
                         const DOUBLE *n_t, const DOUBLE *s_t,
                         DOUBLE *memory_uniforms, DOUBLE *memory_normals,
                         DOUBLE **gammas, cudaStream_t stream = 0) {
  size_t n_loops = GAMMA_LOOPS;
  ...
  mxGPUArray* mx_fill_indices = MEM._mxGPUCreateGPUArray(2, dim_fill,
  mxUINT32_CLASS, mxREAL, MX_GPU_INITIALIZE_VALUES); unsigned int*
  data_fill_indices = (unsigned int*) mxGPUGetData(mx_fill_indices); unsigned
  int** fill_indices = NULL; MEM.cpyToDeviceAsPtrArray(data_fill_indices, m,
  numBlocks.x, fill_indices);*/
  unsigned int **fill_indices = NULL;

  // cudaErrchk(cudaDeviceSynchronize());

  make_gamma2<<<numBlocks, threadsPerBlock,
                m * numBlocks.x * sizeof(unsigned int), stream>>>(
      m, n, n_loops, fill_indices, memory_uniforms, memory_normals, n_t, s_t,
      gammas);
  cudaErrchk(cudaGetLastError());
}
 ...
 */
template <typename DOUBLE>
inline void sampleBeta(curandGenerator_t gen, size_t m, size_t n,
                       const DOUBLE *r_t, const DOUBLE *beta,
                       DOUBLE *memory_uniforms, DOUBLE *memory_normals,
                       DOUBLE **betas, cudaStream_t stream = 0) {
  size_t n_loops = GAMMA_LOOPS;
  ...
  fill_indices = NULL;
  make_beta<<<numBlocks, threadsPerBlock,
              m * numBlocks.x * sizeof(unsigned int), stream>>>(
      m, n, n_loops, fill_indices, memory_uniforms, memory_normals, r_t, beta,
      betas);
  cudaErrchk(cudaGetLastError());
